...
class FieldReader extends Reader {
  ...
  def readBody(json: String, position: Integer, identifier: ListBuffer[Char],
               beyondIdentifier: Boolean = false, beyondColon: Boolean = false): ElementParseResult = {
        ...
        case chr if beyondColon && !whitespace(chr) =>
          chr match {
            ...
            case 't' if json.length > position + 3 && json.substring(position, position + 4) == "true" =>
              new ElementParseResult(new JsonBoolean(stringFromChars(identifier), true), true, position + 4, null)
            case 'f' if json.length > position + 4 && json.substring(position, position + 5) == "false" =>
              new ElementParseResult(new JsonBoolean(stringFromChars(identifier), value = false),true, position + 5, null)
            case 'n' if json.length > position + 3 && json.substring(position, position + 4) == "null" =>
              new ElementParseResult(new JsonNull(stringFromChars(identifier)), true, position + 4, null)
