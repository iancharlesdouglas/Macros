...
import json.{ElementParseResult, JsonElement, JsonObject}
...
class ObjectReader extends Reader {
  ...
  def readBody(json: String, position: Integer, jsonObject: JsonObject): ElementParseResult = {

    json.charAt(position) match {
      case chr if whitespace(chr) => readBody (json, position + 1, jsonObject)
      case '}' => new ElementParseResult(jsonObject, true, position + 1, null)
      case '"' =>
